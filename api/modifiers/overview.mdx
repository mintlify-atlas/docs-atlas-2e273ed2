...
<ParamField path="base_features" type="HolderSet<ConfiguredFeature>" required>
  The configured features to wrap with additional generation
</ParamField>

<ParamField path="stacked_feature" type="PlacedFeature" required>
  The placed feature to add on top of base features
</ParamField>

<ParamField path="placement_type" type="CompositeConfig.Type" default="CANCEL_ON_FAILURE">
  How to handle placement failures: `CANCEL_ON_FAILURE`, `CONTINUE_ON_FAILURE`, or `REQUIRE_ALL`
</ParamField>

<CodeGroup>
```json Stack Feature Example
{
...
<Note>
  See the [Adding Modifiers guide](/guides/adding-modifiers#basic-feature-stacking) for practical examples.
</Note>
